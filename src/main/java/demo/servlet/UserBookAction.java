    ...
    private void borrowBook(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        for (String bookIdString : bookIds) {
            ...
            if (canBorrow(bookId, req, resp)) {
                // DML TRANSACTION
                borrowBook(req, resp, userId, bookId); // INSERT INTO db.table VALUES (?, ?, ...);
                // TODO: 2017/6/16  UPDATE db.table SET amount = amount - 1 WHERE id = bookId;
            } else {
                // TODO: 2017/6/16 通知用户那些书不能借 
            }
        }
